...
void RedeNeural::TreinarRede(const std::string entradas,const std::string saidas_esperadas, int porcentagem_testes, float threshold){
	int num_treinos, num_testes, num_dados, cont_treinos=1;
	...
	do{
		...
		std::cout<<std::endl<<std::endl<<"|INICIANDO TREINAMENTO: "<<cont_treinos<<" |"<<std::endl;
		...
		std::cout<<std::endl<<std::endl<<"|INICIANDO TESTES: "<<cont_treinos<<" |"<<std::endl;
		...
		arquivo_x.clear();
		arquivo_x.seekg(0, ios::beg);
		arquivo_y.clear();
		arquivo_y.seekg(0, ios::beg);
